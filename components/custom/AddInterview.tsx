...
export default function InterviewForm() {
  ...
  const onSubmitFormHandler = async (data: FormSchemaType, buttonName: string) => {
    console.log("Form submitted with data:", data);
    setSubmitLoading(true);
    if (buttonName === "Draft") {
      setSubmitLoadingDraft(true);
    }
    if (buttonName === "Submit") {
      setSubmitLoadingSubmit(true);
    }
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        errornotify(
          `Failed to add the interview: ${error.response?.data?.message || error.message}`
        );
          ...
          {currentStep < 3 ? (
            ...
            <div className="flex gap-2">
              <Button
                ...
                onClick={form.handleSubmit((data) => onSubmitFormHandler(data, "Draft"))}
              >
                {submitLoadingDraft ? (
                  <>
              ...
              <Button
                ...
                onClick={(e) => {
                  ...
                  if (canSubmit) {
                    ...
                    form.handleSubmit((data) => onSubmitFormHandler(data, "Submit"))();
                  }
                }}
              >
                {submitLoadingSubmit ? (
