...
export default function VerificationDialogPopUp() {
  ...
  const rejectInterviewHandler = async () => {
    ...
    try {
      const response = await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/verify/${interviewId.interviewId}/rejected`,
        {},
        {
  ...
  const acceptInterviewHandler = async () => {
    ...
    try {
      const response = await axios.patch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/v1/verify/${interviewId.interviewId}/accepted`,
        {},
        {
          withCredentials: true,
        }
  ...
  return (
    <Dialog>
      ...
      <DialogContent
        ...
        className="sm:max-w-md md:max-w-5xl"
      >
        <DialogHeader>
        ...
        <div className="flex items-center justify-center flex-row space-x-2">
          ...
          <Button onClick={acceptInterviewHandler} variant="default">
            {isAcceptLoading ? (
              <>
                <Loader2 className="mr-2 animate-spin" /> accepting...
              </>
            ) : (
              "Accept"
            )}
